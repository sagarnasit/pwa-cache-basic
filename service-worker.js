...
(function() {
  ...
  self.addEventListener('install', function(event){
    console.log('Attempt to install SW and cache static assets');
    event.waitUntil(
  ...
  self.addEventListener('fetch', function(event) {
    ...
    event.respondWith(
      caches.match(event.request).then(function(response) {
        ...
        .then(function(response){
          if('404' == response.status) {
            console.log('404');
            return caches.match('/cache-api-lab/app/pages/404.html');
          }
          return caches.open(staticCacheName).then(function(cache) {
            if(event.request.url.indexOf('test' < 0)) {
              cache.put(event.request.url, response.clone());
              console.log('Saved in cache',event.request.url);
            }
            return response;
          })
        })
      ...
      .catch(function(err) {
        return caches.match('/cache-api-lab/app/pages/offline.html');
      })
    );
  });
